...
bool PacketMapper::ExpectFlag(fileiter &it, fileiter end, bool &is_negable, bool &is_little_endian, bool &is_float)
{
...
bool PacketMapper::LoadMappings(const std::string &path)
{
    ...
    while (it != end)
    {
        ...
        while (it != end)
        {
            ...
            if (!ExpectFlag(it, end, is_negable, is_little_endian, is_float))
                return false;
